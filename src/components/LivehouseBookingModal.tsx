...
export const LivehouseBookingModal: React.FC<LivehouseBookingModalProps> = ({
  isOpen, onClose, prefillDate, prefillTimeslot, auth, allUsers, livehouseRequests, setLivehouseRequests
}) => {
  ...
  const filteredHosts = allUsers.filter(h => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const currentHost = allUsers.find(h => h.id === hostPoppoId || h.poppo_id === hostPoppoId);
    const managerName = currentHost?.manager || 'Director Miss Nine';
    const managerHost = allUsers.find(h => h.nickname === managerName || h.name === managerName);
              ...
              <fieldset disabled={!isLoggedIn} className="space-y-5 group">
                ...
                <div className="flex items-center gap-3 pt-4">
                  ...
                  <button
                    type="submit"
                    disabled={!isLoggedIn}
                    className={`flex-1 py-3 text-xs font-black uppercase tracking-widest rounded-xl transition-all ${isLoggedIn
