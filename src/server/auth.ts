...
export async function syncCustomClaims(poppoId: string, role: string, tempPasswordRequired: boolean): Promise<void> {
  try {
    ...
    console.log(`âœ… Synced Firebase custom claims for UID '${poppoId}':`, claims);
  } catch (error: any) {
    console.error(`âŒ Failed to sync custom claims for UID '${poppoId}':`, error.message || error);
  }
}
...
setTimeout(async () => {
  try {
    ...
    try {
      ...
      console.log("ðŸ§¹ Startup successfully purged mock performance reports by direct IDs.");
    } catch (cleanErr: any) {
  ...
  try {
    ...
    if (needsMigration && fromFirestore) {
      try {
        ...
        console.log(`âœ… Silently migrated credentials to secure bcrypt hash for Poppo ID: ${poppoId}`);
      } catch (migrationErr) {
 ...
 * Returns { exists, is_first_login } â€” no auth token required.
 */
 ...
 * Phase 2a of the two-step login flow â€” first-time users only.
...
router.post("/set-initial-password", loginRateLimiter, async (req: any, res: any) => {
  ...
  try {
    ...
    try {
      ...
      console.error("âŒ Auth Pipeline Sync Failure:", pipelineError);
...
router.post("/google-login", async (req, res) => {
  try {
    ...
    // Check if the email is an authorized Director â€” link/auto-provision to the master Director account 19157913
    if (email && AUTHORIZED_DIRECTOR_EMAILS.includes(email.toLowerCase())) {
      ...
      if (doc.exists) {
        ...
        console.log(`âœ… Linked existing Director account ${directorId} to Google email: ${email}`);
      } else {
        directorData = {
          id: directorId,
          poppo_id: directorId,
...
router.post("/reset-password", requireAuth(3), async (req: any, res) => {
  try {
    ...
    console.log(`ðŸ” Password reset by ${req.adminUser?.nickname} for Poppo ID: ${poppoId}`);
...
router.post("/update-user", requireAuth(3), async (req: any, res) => {
  try {
    ...
    console.log(`âœï¸ User ${poppoId} updated by ${req.adminUser?.nickname}:`, safeUpdate);
...
router.post("/update-host-profile", verifyFirebaseIdToken, async (req: any, res: any) => {
  try {
    ...
    console.log(`âœï¸ Host Profile ${hostId} updated successfully by ${callerPoppoId}`);
...
router.post("/update-event", verifyFirebaseIdToken, async (req: any, res: any) => {
  try {
    ...
    console.log(`âœï¸ Calendar Event ${eventId} updated by ${callerUid}:`, updatePayload);
    return res.json({ success: true, updated: updatePayload });
  } catch (err: any) {
...
router.post("/delete-event", verifyFirebaseIdToken, async (req: any, res: any) => {
  try {
    ...
    console.log(`ðŸ—‘ï¸ Calendar Event ${eventId} deleted by ${callerUid}`);
...
router.post(
  ...
  async (req: any, res: any) => {
    try {
      ...
      // Step 4: Create Firebase Auth user â€” no password (first-login flow)
      await authInstance.createUser({
        uid: poppoId,
        displayName: nickname,
      });

      // Set custom claims â€” user will set their own password
      ...
      console.log(`ðŸ‘¤ User ${poppoId} created securely by Director ${creatorPoppoId}`);
...
router.delete("/delete-user/:poppoId", verifyAdminRole, async (req: any, res: any) => {
  ...
  try {
    ...
    try {
      ...
      console.log(`ðŸ”¥ Deleted Firebase Auth user: ${cleanPoppoId}`);
    } catch (authErr: any) {
      console.warn(`[DeleteUser Warning]: Auth deletion skipped or failed for ${cleanPoppoId}: ${authErr.message || authErr}`);
    }

    console.log(`ðŸ”¥ User ${cleanPoppoId} deleted by Director/SuperAdmin`);
...
router.post("/reset-account-access", requireAuth(5), async (req: any, res: any) => {
  ...
  try {
    ...
    console.log(`ðŸ”„ Account access reset for ${cleanId} by ${req.adminUser?.poppo_id}`);
  ...
  try {
    ...
    try {
      ...
      if (bucketName) {
        ...
        console.log(`ðŸ’¾ Financials for ${type} saved to Firebase Storage by ${req.adminUser?.nickname}`);
        savedToStorage = true;
      }
    } catch (storageError: any) {
      storageErrorMsg = storageError.message || String(storageError);
      console.warn(`âš ï¸ Failed to save financials to Firebase Storage, falling back to Firestore: ${storageErrorMsg}`);
    }
    ...
    try {
      ...
      console.log(`ðŸ’¾ Financials for ${type} saved to Firestore fallback by ${req.adminUser?.nickname}`);
    } catch (firestoreError: any) {
      console.error("âŒ Failed to save financials to Firestore fallback:", firestoreError);
      if (!savedToStorage) {
  ...
  try {
    ...
    try {
      ...
      if (bucketName) {
        ...
        if (exists) {
          ...
          console.log(`ðŸ“– Financials for ${type} successfully loaded from Firebase Storage`);
          return res.json(data);
        }
      }
    } catch (storageError: any) {
      console.warn(`âš ï¸ Failed to read financials from Firebase Storage, checking Firestore: ${storageError.message || storageError}`);
    }
    ...
    try {
      ...
      if (docSnap.exists) {
        ...
        console.log(`ðŸ“– Financials for ${type} successfully loaded from Firestore flat collection`);
        return res.json(docData?.data || []);
      }
    } catch (firestoreError: any) {
      console.error("âŒ Failed to read financials from Firestore fallback:", firestoreError);
    }
    ...
    if (String(cleanPoppoId) === '19157913' && process.env.DIRECTOR_PASSWORD && String(cleanPassword) === process.env.DIRECTOR_PASSWORD) {
      ...
      if (!hostData) {
        ...
        try {
          ...
          console.log("âœ… Auto-created missing Director doc in Firestore users collection during login-with-poppo bypass.");
        } catch (dbSaveErr) {
...
router.post("/change-password", verifyFirebaseIdToken, async (req: any, res: any) => {
  try {
    ...
    console.log(`ðŸ”’ Password successfully updated for Poppo ID: ${poppoId}`);
    return res.json({
      success: true,
...
router.all("/cleanup-test-reports", requireAuth(5), async (req: any, res: any) => {
  try {
    ...
    if (toDelete.length > 0) {
      ...
      console.log(`ðŸ§¹ API REST Cleaned up ${toDelete.length} test performance reports:`, toDelete);
      return res.json({
        success: true,
